// action types related with user id
export const LOAD_USER_INFO = 'LOAD_USER_INFO';

// action types related with feeds
export const LOAD_FEED = 'LOAD_FEED';
export const LOAD_FEED_RESET = 'LOAD_FEED_RESET';
export const UPDATE_SINGLE_FEED = 'UPDATE_SINGLE_FEED';

// action types related with ProfileMake.js
export const CHANGE_ID = 'CHANGE_ID';
export const CHANGE_NAME = 'CHANGE_NAME';
export const CHANGE_DESC = 'CHANGE_DESC';

// action types related with comment
export const LOAD_COMMENTS = 'LOAD_COMMENTS';
export const SET_LAST_COMMENT = 'SET_LAST_COMMENT';
export const MODIFY_FOLD_COMMENT = 'MODIFY_FOLD_COMMENT';
export const WRITE_COMMENT = 'WRITE_COMMENT';
export const REPLY_COMMENT = 'REPLY_COMMENT';
export const EDIT_COMMENT = 'EDIT_COMMENT';
export const DELETE_COMMENT = 'DELETE_COMMENT';

// action types related with comment
export const LOAD_COMMENT_REPLY = 'LOAD_COMMENT_REPLY';
export const SET_LAST_COMMENT_REPLY = 'SET_LAST_COMMENT_REPLY';
export const MODIFY_FOLD_COMMENT_REPLY = 'MODIFY_FOLD_COMMENT_REPLY';
export const WRITE_COMMENT_REPLY = 'WRITE_COMMENT_REPLY';
export const REPLY_COMMENT_REPLY = 'REPLY_COMMENT_REPLY';
export const EDIT_COMMENT_REPLY = 'EDIT_COMMENT_REPLY';
export const DELETE_COMMENT_REPLY = 'DELETE_COMMENT_REPLY';

// action types related with profile comment
export const LOAD_PROFILE_COMMENT = 'LOAD_PROFILE_COMMENT';
export const SET_LAST_PROFILE_COMMENT = 'SET_LAST_PROFILE_COMMENT';
export const MODIFY_FOLD_PROFILE_COMMENT = 'MODIFY_FOLD_PROFILE_COMMENT';
export const WRITE_PROFILE_COMMENT = 'WRITE_PROFILE_COMMENT';
export const REPLY_PROFILE_COMMENT = 'REPLY_PROFILE_COMMENT';
export const EDIT_PROFILE_COMMENT = 'EDIT_PROFILE_COMMENT';
export const DELETE_PROFILE_COMMENT = 'DELETE_PROFILE_COMMENT';

// action types related with profile comment
export const LOAD_PROFILE_COMMENT_REPLY = 'LOAD_PROFILE_COMMENT_REPLY';
export const SET_LAST_PROFILE_COMMENT_REPLY = 'SET_LAST_PROFILE_COMMENT_REPLY';
export const MODIFY_FOLD_PROFILE_COMMENT_REPLY = 'MODIFY_FOLD_PROFILE_COMMENT_REPLY';
export const WRITE_PROFILE_COMMENT_REPLY = 'WRITE_PROFILE_COMMENT_REPLY';
export const REPLY_PROFILE_COMMENT_REPLY = 'REPLY_PROFILE_COMMENT_REPLY';
export const EDIT_PROFILE_COMMENT_REPLY = 'EDIT_PROFILE_COMMENT_REPLY';
export const DELETE_PROFILE_COMMENT_REPLY = 'DELETE_PROFILE_COMMENT_REPLY';

// action types related with tag
export const LOAD_ARTICLES_TAG = 'LOAD_ARTICLES_TAG';
export const LOAD_PROFILE_TAG = 'LOAD_PROFILE_TAG';
export const RESET_TAG_INFORMATION = 'RESET_TAG_INFORMATION';
export const LOAD_TAG_INFORMATION = 'LOAD_TAG_INFORMATION';
export const LOAD_TAGCLOUD = 'LOAD_TAGCLOUD';
export const INITIALIZE_CANDIDATE_TAGS = 'INITIALIZE_CANDIDATE_TAGS';
export const LOAD_CANDIDATE_TAGS = 'LOAD_CANDIDATE_TAGS';

// action types related with search
export const LOAD_SEARCH_RESULT = 'LOAD_SEARCH_RESULT';

// action types related with updates
export const UPDATE_FOLLOWING = 'UPDATE_FOLLOWING';
export const LOAD_ALL_PROFILES = 'LOAD_ALL_PROFILES';
export const UPDATE_FOLLOWING_STATE = 'UPDATE_FOLLOWING_STATE';

// action types related with profile details
export const LOAD_PROFILE_DETAIL = 'LOAD_PROFILE_DETAIL';
export const CLEAR_PROFILE_DETAIL = 'CLEAR_PROFILE_DETAIL';
export const ERR_PROFILE_DETAIL = 'ERR_PROFILE_DETAIL';

// action types related with profile admin page
export const PROFILE_ERROR_INIT = 'PROFILE_ERROR_INIT';
export const PROFILE_ERROR_NOT_ADMIN = 'PROFILE_ERROR_NOT_ADMIN';
export const PROFILE_ERROR_INVALID_ID = 'PROFILE_ERROR_INVALID_ID';

// action types related with activity
export const LOAD_ACTIVITY = 'LOAD_ACTIVITY';
export const LOAD_ACTIVITY_SUCCESS = 'LOAD_ACTIVITY_SUCCESS';
export const LOAD_ACTIVITY_ERROR = 'LOAD_ACTIVITY_ERROR';

// action types related with profile search
export const SEARCH_PROFILE_WITH_PREFIX_INIT = 'SEARCH_PROFILE_WITH_PREFIX_INIT';
export const SEARCH_PROFILE_WITH_PREFIX_COMPLETE = 'SEARCH_PROFILE_WITH_PREFIX_COMPLETE';

// action types related with modal
export const INITIALIZE_MODAL = 'INITIALIZE_MODAL';
export const DISABLE_MODAL = 'DISABLE_MODAL';
export const ENABLE_MODAL = 'ENABLE_MODAL';
export const SET_MODAL = 'SET_MODAL';

// action types related with article view
export const LOAD_ARTICLE = 'LOAD_ARTICLE';
export const CLEAR_ARTICLE_VIEW = 'CLEAR_ARTICLE_VIEW';

// action types related with realtime
export const UPDATE_TIMES = 'UPDATE_TIMES';

// action types related with measure screen
export const UPDATE_APP_DIMENSIONS = 'UPDATE_APP_DIMENSIONS';
export const UPDATE_MOBILE_STATE = 'UPDATE_MOBILE_STATE';
